...
import { faTag, faAlignLeft, faList, faListAlt, faCube, faDollarSign, faTags, faBoxes, faImages, faWeight, faPaperPlane } from '@fortawesome/free-solid-svg-icons';

function Addproduct() {
    const { id, usertype } = useParams(); // Extract `id` and `usertype` from URL params
    ...
    const handleCategoryChange = (e) => {
        ...
        setSelectedSubcategory(""); // Reset subcategory and item
        setSelectedItem("");
    };
    ...
    const handleImageChange = (event) => {
        ...
        if (file) {
          setSelectedFile(file.name); // Display file name (or file path if you need it in certain cases)
        }
      };
      const handleFormSubmit = async (e) => {
        e.preventDefault();
        setLoading(true);
        const { name, description, category, subcategory, item, price, discountPrice, stockQuantity, weight, images } = productData;
        ...
        const token = localStorage.getItem(id); // Make sure the token exists
        if (!token) {
        ...
        try {
            // Add sellerId to the request body
            const response = await fetch("http://localhost:3000/addproducts", {
                ...
                body: JSON.stringify({
                    ...
                    sellerId: id, // Add sellerId to the body
                }),
            });
